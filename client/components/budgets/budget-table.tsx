...
export function BudgetTable({
    ...
    const observer = new IntersectionObserver(
      (entries) => {
        if (entries[0].isIntersecting && hasMoreResults) {
          setVisibleCount((prev) => Math.min(prev + LOAD_INCREMENT, sortedBudgets.length));
        }
      },
  ...
  }, [visibleCount, hasMoreResults, sortedBudgets.length]);
          ...
          <div className="max-h-[65vh] overflow-y-auto">
            ...
            {hasMoreResults && (
              <div ref={observerTarget} className="px-4 py-8 text-center border-t border-slate-50">
                ...
                <p className="text-[10px] uppercase font-black text-slate-300 tracking-[0.2em] mt-2">Cargando más presupuestos</p>
              </div>
            )}
          </div>
        </div>
