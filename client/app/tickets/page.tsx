...
export default function TicketsPage() {
  ...
  const load = useCallback(() => {
    setIsLoading(true);
    fetch(`${API_URL}/tickets`).then(res => res.json()).then(data => { setTickets(Array.isArray(data) ? data : []); setIsLoading(false); });
  }, []);
    ...
    <DashboardLayout>
      ...
      <div className="flex flex-col bg-slate-50 min-h-full px-4 pt-4 pb-40">
          <div className="flex items-center gap-2 mb-6">
            ...
            <button onClick={() => router.push('/tickets/new')} className="h-14 w-14 bg-amber-500 rounded-2xl flex items-center justify-center text-white shadow-lg active:scale-95 transition-all">
               <Plus size={24} />
            </button>
          </div>
