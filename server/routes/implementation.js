...
const buildImplementationFilter = async (db, clientId) => {
...
router.get('/clients/:id/implementation', async (req, res) => {
  try {
    ...
    const filter = await buildImplementationFilter(db, req.params.id);
    const implementation = await db.collection('client_implementations').findOne(filter);
    if (!implementation) {
...
router.post('/clients/:id/implementation', async (req, res) => {
  try {
    ...
    const filter = await buildImplementationFilter(db, clientId);
