...
app.get('/api/clients/:id/payments', async (req, res) => {
    ...
    if (!mongoDb) return;
    try {
        const rows = await mongoDb.collection('payments').find({ client_id: req.params.id }).sort({ createdAt: -1 }).toArray();
        res.json(rows || []);
    } catch (err) {
        res.status(500).json({ message: err.message });
    }
...
app.get('/api/clients/:id/contracts', async (req, res) => {
    ...
    if (!mongoDb) return;
    try {
        const rows = await mongoDb.collection('contracts').find({ client_id: req.params.id }).sort({ createdAt: -1 }).toArray();
        res.json(rows || []);
    } catch (err) {
        res.status(500).json({ message: err.message });
    }
...
app.get('/api/clients/:id', async (req, res) => {
    ...
    try {
        ...
        const filter = ObjectId.isValid(id) ? { _id: new ObjectId(id) } : { id: parseInt(id) || id };
...
app.put('/api/clients/:id', async (req, res) => {
    ...
    try {
        const { ObjectId } = require('mongodb');
        const filter = ObjectId.isValid(clientId) ? { _id: new ObjectId(clientId) } : { id: parseInt(clientId) || clientId };
