    ...
    if (engine === 'mongodb') {
        try {
            ...
            res.json(tickets.map(t => ({
                ...
                clientId: (t.clientId || t.client_id)?.toString()
            })));
        } catch (error) {
            res.status(500).json({ message: error.message });
        }
    ...
    if (engine === 'mongodb') {
        try {
            ...
            res.json({
                ...
                clientId: (ticket.clientId || ticket.client_id)?.toString()
            });
        } catch (error) {
            res.status(500).json({ message: error.message });
        }
...
app.put('/api/tickets/:id', async (req, res) => {
    ...
    if (engine === 'mongodb') {
        try {
            ...
            const existing = await mongoDb.collection('tickets').findOne({ id: parseInt(ticketId) });
            ...
            // Get updated ticket
            const updatedTicket = await mongoDb.collection('tickets').findOne({ id: parseInt(ticketId) });

            // Handle payment creation if status is 'Facturar'
            if (updatedTicket.status === 'Facturar') {
                // ensurePendingPaymentForTicket logic here if needed
            }
